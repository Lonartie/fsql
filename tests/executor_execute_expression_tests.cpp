...
TEST_CASE("supports arithmetic expressions in WHERE")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT label FROM nums WHERE a + b * 2 = 8;"));
...
TEST_CASE("supports comparison and logical expressions in WHERE")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT label FROM nums WHERE (a > b && a >= 5) || (b > a && b >= 5);"));
...
TEST_CASE("supports bitwise expressions in WHERE")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT label FROM flags WHERE (value & 2) = 2;"));
...
TEST_CASE("supports SELECT subqueries in WHERE expressions")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT title FROM todos WHERE category = (SELECT value FROM defaults);"));
...
TEST_CASE("supports NULL literals in projections and assignments")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT title, archived_at, NULL FROM tasks;"));
...
TEST_CASE("supports IS NULL and IS NOT NULL predicates")
{
    ...
    const auto null_result = context.executor.execute(sql_test::parse_statement("SELECT title FROM tasks WHERE archived_at IS NULL;"));

    const auto& null_table = sql_test::require_table(null_result);
    ...
    const auto not_null_result = context.executor.execute(sql_test::parse_statement("SELECT title FROM tasks WHERE archived_at IS NOT NULL;"));

    const auto& not_null_table = sql_test::require_table(not_null_result);
...
TEST_CASE("distinguishes NULL from empty strings in IS NULL predicates")
{
    ...
    const auto result = context.executor.execute(sql_test::parse_statement("SELECT title FROM tasks WHERE archived_at IS NULL;"));

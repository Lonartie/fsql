...
TEST_CASE("returns failed execution results without throwing from the core executor")
{
    ...
    const auto result = executor.execute(fsql_test::parse_statement("SELECT * FROM missing;"));

    CHECK_FALSE(result.success);
    CHECK_EQ(static_cast<int>(result.kind), static_cast<int>(fsql::ExecutionResultKind::None));
...
TEST_CASE("insert with explicit columns fills unspecified values with empty strings")
{
    ...
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos (title, done) VALUES ('Buy milk', true);"));
...
TEST_CASE("update modifies only matching rows")
{
    ...
    const auto result = context.executor.execute(fsql_test::parse_statement("UPDATE todos SET done = true WHERE title = 'Buy milk';"));
...
TEST_CASE("update without where modifies all rows")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('A', false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('B', false);"));
    const auto result = context.executor.execute(fsql_test::parse_statement("UPDATE todos SET done = true;"));
...
TEST_CASE("drops table from memory storage")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    const auto result = context.executor.execute(fsql_test::parse_statement("DROP TABLE todos;"));
...
TEST_CASE("rejects duplicate table creation")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);")), std::runtime_error);
}
...
TEST_CASE("rejects name collisions between tables and views")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE tasks (title);"));
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("CREATE VIEW tasks AS SELECT title FROM tasks;")), std::runtime_error);

    context.executor.execute(fsql_test::parse_statement("CREATE VIEW open_tasks AS SELECT title FROM tasks;"));
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("CREATE TABLE open_tasks (title);")), std::runtime_error);
}
...
TEST_CASE("rejects unknown columns")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("SELECT missing FROM todos;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("UPDATE todos SET missing = true;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("INSERT INTO todos (missing) VALUES (true);")), std::runtime_error);
}
...
TEST_CASE("alter table rename column keeps data accessible under new name")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, archived_at);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk', '2026-04-22');"));
    context.executor.execute(fsql_test::parse_statement("ALTER TABLE todos RENAME COLUMN archived_at TO closed_at;"));

    const auto result = context.executor.execute(fsql_test::parse_statement("SELECT closed_at FROM todos;"));

    const auto& table = fsql_test::require_table(result);
    REQUIRE_EQ(table.rows.size(), 1U);
    CHECK_EQ(table.rows[0][0], "2026-04-22");
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("SELECT archived_at FROM todos;")), std::runtime_error);
}
...
TEST_CASE("alter table drop column removes stored values")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, category, done);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk', 'home', true);"));
    context.executor.execute(fsql_test::parse_statement("ALTER TABLE todos DROP COLUMN category;"));
...
TEST_CASE("rejects mismatched insert value counts")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk');")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("INSERT INTO todos (title) VALUES ('Buy milk', true);")), std::runtime_error);
}
...
TEST_CASE("rejects invalid alter table operations")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (id AUTO_INCREMENT, title);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES (1, 'Buy milk');"));
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE notes (id, title);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO notes VALUES ('x', 'bad');"));
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE single (only_col);"));

    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("ALTER TABLE todos ADD COLUMN title;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("ALTER TABLE single DROP COLUMN only_col;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("ALTER TABLE todos ALTER COLUMN title SET AUTO_INCREMENT;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("ALTER TABLE notes ALTER COLUMN id SET AUTO_INCREMENT;")), std::runtime_error);
}
...
TEST_CASE("rejects operations on missing tables")
{
    ...
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("SELECT * FROM missing;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("INSERT INTO missing VALUES ('x');")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("UPDATE missing SET value = 1;")), std::runtime_error);
}
...
TEST_CASE("alter view replaces stored definition")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE VIEW open_tasks AS SELECT title FROM tasks WHERE done = false;"));
    context.executor.execute(fsql_test::parse_statement("ALTER VIEW open_tasks AS SELECT title FROM tasks WHERE done = true;"));

    const auto result = context.executor.execute(fsql_test::parse_statement("SELECT title FROM open_tasks;"));
...
TEST_CASE("rejects write operations and mismatched drop commands against views")
{
    ...
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("INSERT INTO open_tasks VALUES ('Patch release');")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("UPDATE open_tasks SET title = 'Done';")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("DELETE FROM open_tasks;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("ALTER TABLE open_tasks ADD COLUMN team;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("DROP TABLE open_tasks;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("DROP VIEW tasks;")), std::runtime_error);
}
...
TEST_CASE("rejects cyclic view definitions and rolls them back")
{
    ...
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("CREATE VIEW loop_view AS SELECT * FROM loop_view;")), std::runtime_error);
...
TEST_CASE("rejects dropping missing table")
{
    ...
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("DROP TABLE missing;")), std::runtime_error);
}
...
TEST_CASE("deletes matching rows with where")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, done);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk', true);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Write docs', false);"));
    const auto result = context.executor.execute(fsql_test::parse_statement("DELETE FROM todos WHERE done = true;"));
...
TEST_CASE("qualified WHERE predicates behave consistently across select update and delete")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE tasks (title, team_id, priority, done);"));
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE teams (id, name);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES ('Patch release', 10, 8, false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES ('Write docs', 20, 8, false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES ('Archive logs', 10, 3, false);"));
    ...
    const auto& selected = fsql_test::require_table(select_result);
    REQUIRE_EQ(selected.rows.size(), 1U);
    CHECK_EQ(selected.rows[0][0], "Patch release");

    const auto update_result = context.executor.execute(fsql_test::parse_statement(
    ...
    const auto delete_result = context.executor.execute(fsql_test::parse_statement(
...
TEST_CASE("single table update and delete reject unknown qualified WHERE identifiers")
{
    ...
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("UPDATE tasks SET done = true WHERE missing.done = false;")), std::runtime_error);
    CHECK_THROWS_AS(context.executor.execute(fsql_test::parse_statement("DELETE FROM tasks WHERE missing.done = false;")), std::runtime_error);
}
...
TEST_CASE("update and delete support inline IN list predicates")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE tasks (id, title, done);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (1, 'Patch release', false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (2, 'Write docs', false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (3, 'Rotate keys', false);"));
...
TEST_CASE("update and delete support NOT IN predicates")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE tasks (id, title, team_id, done);"));
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE teams (id, name);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (1, 'Patch release', 10, false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (2, 'Write docs', 20, false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO tasks VALUES (3, 'Rotate keys', 30, false);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO teams VALUES (10, 'ops');"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO teams VALUES (30, 'sec');"));
...
TEST_CASE("update can assign NULL values")
{
    ...
    context.executor.execute(fsql_test::parse_statement("CREATE TABLE todos (title, archived_at);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk', '2026-04-22');"));
    context.executor.execute(fsql_test::parse_statement("UPDATE todos SET archived_at = NULL WHERE title = 'Buy milk';"));

    const auto result = context.executor.execute(fsql_test::parse_statement("SELECT title FROM todos WHERE archived_at IS NULL;"));
...
TEST_CASE("deletes all rows without where")
{
    ...
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Buy milk', true);"));
    context.executor.execute(fsql_test::parse_statement("INSERT INTO todos VALUES ('Write docs', false);"));
    const auto result = context.executor.execute(fsql_test::parse_statement("DELETE FROM todos;"));
...
TEST_CASE("drops views from memory storage")
{
    ...
    const auto result = context.executor.execute(fsql_test::parse_statement("DROP VIEW open_tasks;"));
...
TEST_CASE("csv storage supports quoted file paths for table statements")
{
    ...
    const auto loaded = storage->load_table({fsql::RelationReference::Kind::FilePath, table_path_text});
    ...
    CHECK(storage->load_table({fsql::RelationReference::Kind::FilePath, table_path_text}).rows.empty());
...
TEST_CASE("csv storage supports quoted file paths for view statements")
{
    ...
    const auto view = storage->load_view({fsql::RelationReference::Kind::FilePath, view_path_text});

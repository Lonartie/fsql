...
namespace fsql
{
    /// @brief Persists tables in TOML files.
    class TomlStorage final : public IStorage
    {
    public:
        TomlStorage();
        explicit TomlStorage(std::filesystem::path root_directory);
    ...
    private:
        ...
        FSQL_AUTO_REGISTER_STORAGE(TomlStorage, ".toml");
    };
}

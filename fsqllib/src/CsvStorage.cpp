...
namespace fsql
{
    ...
    bool CsvStorage::supports_append(const RelationReference& table_name) const
    {
        return storage_support_.supports_append(table_name);
    }

    void CsvStorage::append_row(const RelationReference& table_name, const Table& table, const Row& row)
    {
        storage_support_.append_row(table_name, table, row);
    }

    std::string CsvStorage::next_auto_increment_value_for_insert(const RelationReference& table_name,
                                                                 const Table& table,

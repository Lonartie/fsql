...
namespace fsql
{
    namespace
    {
        ...
        void write_sql_help(std::ostream& output)
        {
            output
                << version() << '\n'
                << "Usage: fsql [options] [SQL statement]\n\n";
            write_common_options(output);
            write_sql_syntax_overview(output);
            output
                << "Examples:\n"
                << "  fsql \"SELECT * FROM todos;\"\n"
                << "  fsql \"SELECT title FROM todos WHERE done = false ORDER BY title;\"\n"
                << "  fsql \"SELECT team, COUNT(*) FROM tasks GROUP BY team HAVING COUNT(*) >= 2;\"\n"
                << "  fsql \"SELECT src.title FROM '/tmp/tasks.csv' src WHERE src.done = false;\"\n"
                << "  fsql \"INSERT INTO '/tmp/tasks.csv' (title, done) VALUES ('Buy milk', false);\"\n"
                << "  fsql \"INSERT INTO todos (title, done) VALUES ('Buy milk', false);\"\n"
                << "  fsql \"UPDATE todos SET done = true WHERE title = 'Buy milk';\"\n"
                << "  fsql \"DELETE FROM todos WHERE done = true;\"\n"
                << "  fsql \"CREATE TABLE todos (id AUTO_INCREMENT, title, done = false);\"\n"
                << "  fsql \"CREATE VIEW '/tmp/open_todos' AS SELECT title FROM '/tmp/todos.csv' WHERE done = false;\"\n"
                << "  fsql \"CREATE VIEW open_todos AS SELECT title FROM todos WHERE done = false;\"\n"
                << "  fsql \"DROP VIEW open_todos;\"\n";
        }
    ...
    {
        if (program_name == "fsql" || program_name == "sql")
        {
